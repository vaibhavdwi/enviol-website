...
export default function PIRFoamPolyolPage() {
  ...
  const pirProduct = products.find(
    (p) => p.product_code === "RENVIOL-PolyRig-2800"
  );
        ...
        {pirProduct && (
          <div className="bg-white rounded-xl p-6 shadow border border-gray-100 hover:shadow-xl transition-all duration-300">
            ...
            <h3 className="text-xl font-bold text-gray-800">
              {pirProduct.product_name}
            </h3>

            <p className="text-sm text-gray-500 mt-1 font-medium">
              {pirProduct.product_code}
            </p>

            <p className="mt-3 text-sm text-gray-700 leading-relaxed">
              {pirProduct.short_description}
            </p>
            ...
            <div className="mt-4 space-y-2 text-sm text-gray-700">
              ...
              <p>
                ...
                {pirProduct.applications?.join(", ")}
              </p>

              <p>
                <span className="font-semibold text-gray-800">Viscosity:</span>{" "}
                {pirProduct.technical_data?.dynamic_viscosity || "-"}
              </p>

              <p>
                <span className="font-semibold text-gray-800">Hydroxyl Value:</span>{" "}
                {pirProduct.technical_data?.hydroxyl_number || "-"}
              </p>

            </div>

            <Link
              href={`/products/${pirProduct.slug}`}
