        ...
        <div className="space-y-6">
          ...
          <Section title="Executive Summary">
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
              ...
              <Card title="Total Events" value={report?.summary?.totalEvents || 0} />
              <Card title="Visitors" value={report?.summary?.uniqueVisitors || 0} />
              <Card title="Sessions" value={report?.summary?.sessions || 0} />
              <Card title="Returning" value={report?.summary?.returningVisitors || 0} />
              <Card title="Countries" value={report?.summary?.countries || 0} />
              <Card title="Regions" value={report?.summary?.regions || 0} />
          ...
          <Section title="Engagement Metrics">
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
              ...
              <Card title="Pages / Visitor" value={report?.summary?.avgPagesPerVisitor || 0} />
              <Card title="CTA Clicks" value={report?.summary?.ctaClicks || 0} />
              <Card title="Navigation" value={report?.summary?.navigationClicks || 0} />
              <Card title="Forms" value={report?.summary?.formSubmits || 0} />
          ...
          <Section title="Visitor Analytics">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card title="Unique Visitors" value={report?.summary?.uniqueVisitors || 0} />
              <Card title="Returning Visitors" value={report?.summary?.returningVisitors || 0} />
              <Card title="Sessions" value={report?.summary?.sessions || 0} />
              <Card title="Pages / Visitor" value={report?.summary?.avgPagesPerVisitor || 0} />
          ...
          <Section title="Geography">
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <TableCard
                ...
                render={(row) => (
                  <>
                    <td className="p-2 border">{row.country || "Unknown"}</td>
                    <td className="p-2 border">{row.visitors || 0}</td>
                  </>
                )}
              />
              ...
              <TableCard
                ...
                render={(row) => (
                  <>
                    <td className="p-2 border">{row.region || "Unknown"}</td>
                    <td className="p-2 border">{row.visitors || 0}</td>
                  </>
                )}
              />
              ...
              <TableCard
                ...
                render={(row) => (
                  <>
                    <td className="p-2 border">{row.city || "Unknown"}</td>
                    <td className="p-2 border">{row.visitors || 0}</td>
                  </>
                )}
              />
          ...
          <Section title="Top Pages">
            <Table
              ...
              render={(row) => (
                <>
                  <td className="p-2 border break-all">{row.page || "-"}</td>
...
function Card({ title, value }) {
  return (
    <div className="bg-white border rounded-lg shadow-sm p-4">
      ...
      <p className="text-2xl font-bold text-[#1F524F] mt-2">{value}</p>
    </div>
  );
}
...
function TableCard({ title, headers, data, render }) {
  return (
    <div>
      <h3 className="font-semibold mb-3">{title}</h3>
      <Table headers={headers} data={data} render={render} />
    </div>
  );
}
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm border">
        ...
        <tbody>
          {data.length === 0 ? (
            <tr>
              <td className="p-3 border text-gray-500" colSpan={headers.length}>
                No data available
              </td>
            </tr>
          ) : (
