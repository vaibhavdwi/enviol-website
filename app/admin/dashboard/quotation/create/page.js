...
export default function QuotationPage() {
  ...
  try {
    ...
    // ✅ 1. SAVE TO DATABASE
    const saveRes = await fetch("/api/admin/quotation/create", {
      method: "POST",
      headers: {
    ...
    // ✅ 2. GENERATE PDF (existing API)
    const pdfRes = await fetch("/api/admin/quotation/pdf", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    a.href = url;
    a.download = `quotation_${saveData.quotation_number || "new"}.pdf`;
    a.click();
